...
python3 predict.py --closing              # capture the closing line (run near first pitch)
```
...
If you set an `ODDS_API_KEY` environment variable (free tier at
the-odds-api.com — 500 requests/month), every `--all` run also captures the
current consensus moneyline for each game and logs it as the **opening**
line, alongside the model's own probability. Running
`python3 predict.py --closing` again later the same day (ideally close to
first pitch) captures the market one more time as the **closing** line — the
free tier only exposes the live line, not historical replays, so "opening"
and "closing" are just two snapshots taken at different times.

Once a pick settles, the log also gets: the vig-free (de-vigged) win
probability implied by the closing line, our closing-line value (CLV — did
the market move toward our pick after we logged it?), whether our model's
own probability beat the vig-free closing price (+EV), and profit/loss on a
flat 1-unit bet at the opening price. **market.html** (rebuilt each run,
once there are settled picks with a full odds snapshot) shows average CLV,
% of picks with positive CLV, and flat-stake ROI — overall and restricted to
picks where our edge over the closing line exceeded a threshold.

Without `ODDS_API_KEY` set, none of this runs and nothing changes — the odds
columns in picks_log.csv just stay blank.
...
market.html                       CLV & ROI vs. the market (needs ODDS_API_KEY)
